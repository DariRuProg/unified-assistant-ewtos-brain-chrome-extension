# Chrome Web Store — Listing & Submission-Checkliste (Ewtos Office-Brain)
...
> Ewtos Office-Brain ist ein Produktivitäts-Assistent, der Web-Inhalte erfasst und mit einer selbst betriebenen Wissens-Datenbank (Obsidian-Vault) per KI nutzbar macht. Alle Funktionen dienen diesem einen Zweck: Wissen erfassen, organisieren und befragen.

## Titel & Kurzbeschreibung
- **Titel (≤ ~45 Zeichen, Keyword zuerst):** „Ewtos Office-Brain — Obsidian KI-Assistent"
...
> Sprich direkt aus dem Browser mit deinem Obsidian-Vault: Ewtos Office-Brain verbindet eine Chrome-Extension mit deinem eigenen, lokal oder auf deinem Server laufenden Ewtos Office-Brain-Backend. Deine Notizen, dein LLM (BYOK: Claude, OpenAI, Mistral, Ollama lokal), deine Kontrolle — keine Daten in fremder Cloud.
>
...
>
> **Wichtig — Companion-Server:** Ewtos Office-Brain braucht das kostenlose Ewtos Office-Brain-Backend (Python). Installiere es lokal (Setup-Download auf ewtos.com) oder verbinde dich mit einem Server deiner Firma. Ohne Server zeigt die Extension eine Setup-Anleitung. Zum risikolosen Antesten gibt es eine öffentliche Demo-Instanz.
>
...
| **Host (Pflicht)** `http(s)://localhost`, `http(s)://127.0.0.1` | Verbindung (WebSocket + REST) zum lokal laufenden Ewtos Office-Brain-Backend. |
...
- **Weitergabe:** Daten gehen nur an das vom Nutzer konfigurierte Ewtos Office-Brain-Backend (lokal/eigener Server) und an den vom Nutzer gewählten LLM-Anbieter (BYOK). Ewtos Office-Brain/ewtos.com erhält keine Inhalte.

## Assets
- [ ] Icon 128×128 (vorhanden: `extension/images/icon-128.png`).
- [x] 3–5 Screenshots 1280×800 in `docs/store-assets/out/`: `01-main` (Split-View), `02-explorer` (Vault+Markdown), `03-chat` (Vault-Chat), `04-scrape` (Web-Tool), `05-video`/`00-intro`. Aus der Live-Demo aufgenommen.
- [ ] Optional: Promo-Kachel.
- [ ] Privacy-Policy-URL öffentlich: `https://ewtos.com/office-brain/datenschutz` (Inhalt aus `docs/legal/privacy-policy.md`).
...
- [ ] Datenschutzerklärung online (`ewtos.com/office-brain/datenschutz`); Impressum via `ewtos.com/impressum` (bestehend, gleiche juristische Person).
